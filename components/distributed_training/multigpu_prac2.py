...
class Trainer:
    def __init__(self, rank, dataloader, model, optimizer, total_epochs, save_every):
        self.rank = rank
...
def main(rank, world_size, batch_size, learning_rate, total_epochs, save_every):
    ddp_init(world_size, rank)
    dataloader, model, optimizer = get_train_objs(batch_size, learning_rate)
    trainer = Trainer(rank, dataloader, model, optimizer, total_epochs, save_every)
    trainer.train()
