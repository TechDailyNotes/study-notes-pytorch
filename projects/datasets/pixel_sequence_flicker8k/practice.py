...
class FlickerDataset(Dataset):
    def __init__(
        self, images_folder, captions_file, frequency_threshold,
        transform=None,
    ):
    ...
    def __getitem__(self, index):
        ...
        image = Image.open(
            os.path.join(self.images_folder, image_index)
        ).convert('RGB')
        if self.transform:
...
def get_dataloader():
    ...
    dataset = FlickerDataset(
        images_folder, captions_file, frequency_threshold, transform,
    )
